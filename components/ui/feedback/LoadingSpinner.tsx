    ...
    return (
        <div className={`flex flex-col items-center justify-center ${text ? 'p-8 gap-4' : 'p-4'}`}>
            <div className="relative">
                ...
                <div className={`absolute inset-0 ${sizeClasses[size]} border-indigo-500/0 border-t-indigo-500 rounded-full animate-spin blur-[2px] opacity-50`} />
            </div>
            {text && <p className="text-xs font-bold text-slate-500 uppercase tracking-widest animate-pulse">{text}</p>}
        </div>
    )
}
