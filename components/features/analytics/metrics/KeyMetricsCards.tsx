...
export function KeyMetricsCards({ currentPeriod, previousPeriod, compareMode, loading = false }: KeyMetricsCardsProps) {
  ...
  const getChangeColor = (change: number, isExpense = true) => change === 0 ? 'text-zinc-400' : (isExpense ? (change < 0 ? 'text-emerald-400' : 'text-rose-400') : (change > 0 ? 'text-emerald-400' : 'text-rose-400'))
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
        {[1, 2, 3, 4].map(i => (
          <div key={i} className="bg-zinc-800 p-6 rounded-xl border border-zinc-700 shadow-lg flex flex-col items-center justify-center min-h-[140px]">
            <div className="w-10 h-10 bg-zinc-700 rounded-full mb-3 animate-pulse" />
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
      {metrics.map(metric => {
        ...
        return (
          <div key={metric.title} className={`bg-zinc-800 p-6 rounded-xl border border-zinc-700 border-t-4 ${metric.borderClass} shadow-lg flex flex-col items-center justify-center min-h-[140px]`}>
            ...
            <h3 className="text-sm font-semibold text-zinc-400 mb-2 text-center">{metric.title}</h3>
            ...
            {compareMode && previousPeriod && metric.change !== undefined && (
              <div className="flex flex-col items-center gap-1 p-2 bg-zinc-700 rounded-lg border border-zinc-600">
