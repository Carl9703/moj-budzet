...
const PREV_COLOR = '#f97316'   // orange-500 — poprzedni rok
const CURR_COLOR = '#fb7185'   // rose-400 — bieżący rok
...
export function YearComparisonChart({
    ...
    const chartData = useMemo(() =>
        currentYearData.map((current, index) => ({
            ...
            curr: current.expenses,
            prev: previousYearData?.[index]?.expenses ?? 0,
        })),
        [currentYearData, previousYearData]
    )

    const totalCurr = useMemo(() => currentYearData.reduce((s, d) => s + d.expenses, 0), [currentYearData])
    const totalPrev = useMemo(() => previousYearData?.reduce((s, d) => s + d.expenses, 0) ?? 0, [previousYearData])
