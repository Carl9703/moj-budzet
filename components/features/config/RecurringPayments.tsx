...
export function RecurringPayments({ envelopes }: RecurringPaymentsProps) {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center p-12 gap-4">
                ...
                <p className="text-zinc-400 text-sm animate-pulse">Ładowanie płatności cyklicznych...</p>
            </div>
        )
    }
    ...
    return (
        <div className="p-1">
            {/* Header */}
            <div className="flex flex-col sm:flex-row justify-between items-center mb-8 p-6 rounded-3xl border border-white/5 bg-zinc-900/50 backdrop-blur-xl gap-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-zinc-500 font-medium">Zarządzaj automatycznymi przypomnieniami</p>
                    </div>
                </div>
                <button
            ...
            {payments.length === 0 ? (
                <div className="text-center p-12 glass-card-static flex flex-col items-center">
                    ...
                    <p className="text-zinc-400 text-sm">Dodaj pierwszą płatność, aby system pamiętał o Twoich rachunkach.</p>
                </div>
            ) : (
                    ...
                    <AnimatePresence>
                        {payments.map((payment, index) => (
                            <motion.div
                                ...
                                className="p-5 rounded-3xl border border-white/5 bg-zinc-900/50 backdrop-blur-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6 hover:bg-zinc-800/80 hover:border-white/10 transition-all group shadow-xl"
                            >
                                <div className="flex items-start gap-4 flex-1">
                                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shrink-0 shadow-inner ${payment.isActive ? (payment.type === 'transfer' ? 'bg-purple-500/10 text-purple-400' : 'bg-amber-500/10 text-amber-400') : 'bg-zinc-900 text-zinc-600'}`}>
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <div className="flex items-center gap-3 mb-1.5 mt-1">
                                            ...
                                            {!payment.isActive && <span className="text-[8px] font-black bg-zinc-800 text-zinc-500 px-1.5 py-0.5 rounded-md border border-white/5 uppercase tracking-widest">NIEAKTYWNA</span>}
                                        </div>
                                ...
                                <div className="flex gap-2 w-full md:w-auto justify-end opacity-100 md:opacity-0 group-hover:opacity-100 transition-all">
                                    <button onClick={() => handleEdit(payment)} className="w-10 h-10 bg-zinc-900 hover:bg-amber-600 hover:text-white rounded-xl transition-all text-zinc-500 flex items-center justify-center border border-white/5 shadow-inner">
                                        <Edit size={16} />
                                    </button>
                                    <button onClick={() => handleDelete(payment.id)} className="w-10 h-10 bg-zinc-900 hover:bg-rose-600 hover:text-white rounded-xl transition-all text-zinc-500 flex items-center justify-center border border-white/5 shadow-inner">
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                            </motion.div>
