...
export function TransactionFilters({ onFiltersChange, filterOptions, loading, initialFilters }: TransactionFiltersProps) {
  ...
  const FilterTag = ({ label, onClear }: { label: string; onClear: () => void }) => (
    <motion.span
      ...
      className="inline-flex items-center gap-1.5 bg-zinc-800/50 border border-zinc-700 rounded-full py-1.5 px-3 text-xs text-zinc-300 backdrop-blur-sm"
    >
      {label}
      <button onClick={onClear} className="border-none bg-transparent cursor-pointer text-zinc-400 hover:text-white transition-colors">
        <X size={14} />
      </button>
    </motion.span>
  )
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_200px_auto] gap-4 items-center p-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            disabled={loading}
            className="w-full bg-zinc-950/50 border border-white/10 rounded-2xl !pl-12 py-3 text-sm font-black text-white placeholder:text-zinc-700 focus:outline-none focus:border-amber-500/50 transition-all shadow-inner disabled:opacity-50"
          />
        </div>
      ...
      <AnimatePresence>
        {activeFiltersCount > 0 && (
          <motion.div
            ...
            className="flex flex-wrap gap-2 py-3 px-4 border-b border-[rgba(255,255,255,0.05)] bg-zinc-900/20"
          >
      ...
      <AnimatePresence>
        {isAdvancedOpen && (
          ...
          >
            <div className="p-4 border-t border-[rgba(255,255,255,0.05)] bg-zinc-900/10">
              ...
              <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-[rgba(255,255,255,0.05)]">
                <span className="text-sm font-medium text-zinc-400">Sortowanie:</span>
