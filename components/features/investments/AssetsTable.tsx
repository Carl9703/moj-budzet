...
export const AssetsTable = ({ assets, totalPortfolioValue, onEdit, onDelete }: AssetsTableProps) => {
    ...
    return (
        <motion.div
            ...
            className="p-6 rounded-[2.5rem] border border-white/5 bg-slate-900/50 backdrop-blur-xl"
        >
            <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[10px] font-black text-slate-400 bg-slate-800/50 px-3 py-1.5 rounded-xl border border-white/5 uppercase tracking-widest">
                    {assets.length} {assets.length === 1 ? 'Pozycja' : assets.length < 5 ? 'Pozycje' : 'Pozycji'}
                </span>
            </div>
            <p className="text-sm text-slate-400 mb-8 ml-14">
                Zarządzaj swoim portfelem inwestycyjnym i śledź wyniki poszczególnych aktywów.
            </p>

            <div className="space-y-4">
                {assets.length === 0 ? (
                    <div className="p-12 text-center bg-slate-800/20 rounded-3xl border border-white/5">
                        <div className="w-16 h-16 bg-slate-800/50 rounded-2xl flex items-center justify-center mx-auto mb-4 opacity-50">
                            <Briefcase size={32} className="text-slate-400" />
                        </div>
                        <p className="text-slate-500 font-medium">Brak aktywów w Twoim portfelu</p>
                    </div>
                ) : (
                    groupKeys.map((group, index) => {
                        ...
                        return (
                            <div
                                ...
                                className={`rounded-3xl border transition-all duration-300 overflow-hidden ${isExpanded ? 'border-indigo-500/30 bg-indigo-500/5 shadow-xl' : 'border-white/5 bg-slate-800/20 hover:bg-slate-800/40'}`}
                            >
                                <button
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl transition-colors shadow-inner ${isExpanded ? 'bg-indigo-500 text-white' : 'bg-slate-800 text-slate-400'}`}>
                                            {getGroupIcon(group)}
                                        </div>
                                        <div className="text-left">
                                            <p className={`font-black text-sm uppercase tracking-widest transition-colors ${isExpanded ? 'text-indigo-200' : 'text-slate-200'}`}>
                                                {group}
                                            </p>
                                            <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-tighter text-slate-500 mt-1">
                                                <span>{groupAssets.length} {groupAssets.length === 1 ? 'poz.' : 'poz.'}</span>
                                                <span className="w-1 h-1 rounded-full bg-slate-700" />
                                                <span className="text-slate-400">{groupPercentage.toFixed(1)}% portfela</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-6">
                                        <div className="text-right">
                                            <p className="font-black text-slate-100 text-lg tabular-nums tracking-tighter">
                                        ...
                                        <div className={`p-2 rounded-xl transition-colors ${isExpanded ? 'bg-indigo-500/20 text-indigo-400' : 'bg-white/5 text-slate-600'}`}>
...
const AssetItem = ({
    ...
    return (
        <div className="group rounded-2xl bg-slate-900/40 border border-white/5 overflow-hidden transition-all hover:border-white/10">
            <div className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                            ...
                            {hasSubPositions && (
                                <span className="bg-slate-800 text-[9px] font-black px-2 py-0.5 rounded-full text-slate-500 uppercase tracking-widest">
                ...
                <div className="flex items-center gap-6">
                    <div className="text-right">
                        <p className="font-black text-slate-200 text-base tabular-nums tracking-tighter">
                    ...
                    <div className="flex gap-1">
                        <button
                            ...
                            className="w-8 h-8 flex items-center justify-center bg-slate-800/50 hover:bg-slate-700 border border-white/5 rounded-xl text-slate-500 hover:text-indigo-400 transition-all active:scale-90"
                            title="Edytuj"
                        >
                            <Pencil size={14} />
                        </button>
                        <button
                            ...
                            className="w-8 h-8 flex items-center justify-center bg-slate-800/50 hover:bg-rose-900/30 border border-white/5 rounded-xl text-slate-500 hover:text-rose-400 transition-all active:scale-90"
                            title="Usuń"
                        >
                            <Trash2 size={14} />
                        </button>
            ...
            <AnimatePresence>
                {isExpanded && hasSubPositions && (
                    <motion.div
                        ...
                        animate={{ height: 'auto', opacity: 1 }}
                        exit={{ height: 0, opacity: 0 }}
                        className="bg-slate-900/60 border-t border-white/5"
                    >
                        <div className="p-3 pl-20 space-y-2">
                            <p className="text-[9px] font-black text-slate-600 uppercase tracking-[0.2em] mb-3">Historia transakcji</p>
                            {asset.subPositions?.map((sub, idx) => (
                                <div
                                    key={sub.id}
                                    className="flex items-center justify-between p-2.5 rounded-xl bg-slate-800/30 border border-white/[0.02] hover:bg-slate-800/50 transition-colors"
                                >
                                    <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <button
                                            ...
                                            className="p-1.5 hover:bg-rose-500/10 rounded-lg text-slate-700 hover:text-rose-500 transition-colors"
