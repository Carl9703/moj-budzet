...
const colorMap: Record<string, { bg: string; border: string; text: string }> = {
    ...
    indigo: { bg: 'bg-indigo-500/5', border: 'border-indigo-500/10', text: 'text-indigo-400' },
    ...
    rose: { bg: 'bg-rose-500/5', border: 'border-rose-500/10', text: 'text-rose-400' },
...
export const StatsCard = ({ title, value, icon: Icon, trend, subtitle, colorClass = "indigo", className = "", onClick, index = 0 }: StatsCardProps) => {
    ...
    return (
        <motion.div
            ...
            onClick={onClick}
            className={`py-3 px-6 rounded-[32px] bg-slate-900/40 backdrop-blur-xl border border-white/5 flex flex-col relative overflow-hidden group transition-all duration-300 ${className} ${onClick ? 'cursor-pointer hover:bg-slate-800/40 hover:scale-[1.02] hover:border-white/10' : ''}`}
        >
            {/* Minimal Header */}
            ...
            <div className="flex flex-col items-start w-full">
                {typeof value === 'string' ? (
                    <div className="text-[26px] md:text-[36px] font-black text-white tracking-tighter leading-none mb-1">
                        {value}
                    </div>
                ) : (
                ...
                {trend && (
                    <div className="flex items-center gap-2 mt-2">
                        ...
                        <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">{trend.label}</span>
                    </div>
                )}

                {subtitle && (
                    <p className="text-[11px] text-slate-500 font-medium tracking-tight opacity-80 leading-relaxed text-left">
                        {subtitle}
                    </p>
                )}
            </div>
