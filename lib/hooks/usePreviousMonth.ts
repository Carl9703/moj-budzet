...
import { SYSTEM_DESCRIPTIONS } from '../constants/system'
    ...
    useEffect(() => {
        const checkPreviousMonth = async () => {
            try {
                ...
                const checkResponse = await api.get<{ id: string; description: string | null; date: string }[]>('/api/transactions')

                const currentMonthStart = new Date(now.getFullYear(), now.getMonth(), 1)
                const firstDaysOfCurrentMonth = new Date(now.getFullYear(), now.getMonth(), 3, 23, 59, 59)

                const isClosed = checkResponse.some((t) => {
                    if (!t.description?.includes(SYSTEM_DESCRIPTIONS.MONTH_CLOSE)) {
                        return false
                    }
